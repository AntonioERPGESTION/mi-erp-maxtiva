...
# Definición maestra de pestañas y columnas
ESTRUCTURA_BBDD = {
...
def asegurar_y_obtener_datos(sh, nombre_hoja):
    ...
    try:
        ...
        # Asegurar que todas las columnas necesarias existan para evitar KeyError/AttributeError
...
with st.sidebar:
    ...
    seccion = st.selectbox("Seleccionar Módulo", ["📊 Dashboard"] + list(ESTRUCTURA_BBDD.keys()))
...
if seccion == "📊 Dashboard":
    ...
    if not df_o.empty:
        ...
        for col in cols_gasto:
            if col not in df_i.columns: df_i[col] = 0.0
        ...
        st.info("No hay obras registradas para mostrar estadísticas.")

# --- IMPUTACIONES (CON LÓGICA DE 11H) ---
elif seccion == "Imputaciones":
    st.header("🕒 Registro de Jornada y Gastos")
    df_i, ws_i = asegurar_y_obtener_datos(sh, "Imputaciones")
    df_o, _ = asegurar_y_obtener_datos(sh, "Obras")
    df_e, _ = asegurar_y_obtener_datos(sh, "Empleados")
    
    with st.expander("📝 Nueva Imputación Diaria"):
        with st.form("form_imp"):
            c1, c2, c3 = st.columns(3)
            f = c1.date_input("Fecha", datetime.now())
            t = c2.selectbox("Trabajador", df_e['NOMBRE'].unique() if not df_e.empty else ["No hay empleados"])
            o = c3.selectbox("Obra", df_o['NOMBRE'].unique() if not df_o.empty else ["No hay obras"])
            
            # Lógica de horas: 8 normales + 3 extras
            h_tot = st.slider("Horas Totales Trabajadas", 0.0, 11.0, 8.0)
            
            c4, c5, c6, c7 = st.columns(4)
            dietas = c4.number_input("Dietas (€)", 0.0)
            gas = c5.number_input("Gasolina (€)", 0.0)
            peajes = c6.number_input("Peajes (€)", 0.0)
            ora = c7.number_input("ORA/Parking (€)", 0.0)
            
            if st.form_submit_button("Guardar Registro"):
                ws_i.append_row([str(f), t, o, h_tot, dietas, gas, peajes, ora, 0])
...
# --- MÓDULOS RESTANTES ---
    ...
    # Configuración de fecha automática si la columna existe
    config = {c: st.column_config.DateColumn() for c in df.columns if "FECHA" in c}
